...
class CountriesApp extends React.Component{
    ...
    filterCountriesByRegion=(region)=>{
        if(region==='All'){
            this.setState({filteredCountries:this.state.searchedCountries});
            this.setState({regionalCountries:this.state.countries});
        }else{
        this.setState({filteredCountries:this.state.searchedCountries.filter(each=>region===each.region),
    ...
    filterCountriesByName=(text)=>{
        if(text.length){
        this.setState({filteredCountries:this.state.regionalCountries.filter(each=>(each.name.toLowerCase().includes(text.toLowerCase()))),
                        searchedCountries:this.state.countries.filter(each=>(each.name.toLowerCase().includes(text.toLowerCase())))
        });
        }
        else{
            this.setState({filteredCountries:this.state.regionalCountries});
            this.setState({searchedCountries:this.state.countries.filter(each=>(each.name.toLowerCase().includes(text.toLowerCase())))});
        }
    }
